/**
 * Сообщения react-admin на русском (вендоринг по лицензии MIT пакета ra-language-russian 5.4.5),
 * чтобы не зависеть от npm-пакета в Docker / устаревшего node_modules.
 */
const russianMessages = {
  ra: {
    action: {
      add_filter: 'Добавить фильтр',
      add: 'Добавить',
      back: 'Назад',
      bulk_actions: '1 выбран |||| %{smart_count} выбрано |||| %{smart_count} выбрано',
      cancel: 'Отмена',
      clear_array_input: 'Очистить список',
      clear_input_value: 'Очистить',
      clone: 'Дублировать',
      confirm: 'Подтвердить',
      create: 'Создать',
      create_item: 'Создать %{item}',
      delete: 'Удалить',
      edit: 'Редактировать',
      export: 'Экспорт',
      list: 'Список',
      refresh: 'Обновить',
      remove_filter: 'Убрать фильтр',
      remove_all_filters: 'Убрать все фильтры',
      remove: 'Удалить',
      reset: 'Сбросить',
      save: 'Сохранить изменения',
      search: 'Поиск',
      search_columns: 'Поиск столбцов',
      select_all: 'Выбрать все',
      select_all_button: 'Выбрать все',
      select_row: 'Выбрать эту запись',
      show: 'Просмотр',
      sort: 'Сортировка',
      undo: 'Отменить',
      unselect: 'Не выбрано',
      expand: 'Раскрыть',
      close: 'Закрыть',
      open_menu: 'Открыть меню',
      close_menu: 'Закрыть меню',
      update: 'Обновить',
      move_up: 'Переместить вверх',
      move_down: 'Переместить вниз',
      open: 'Открыть',
      toggle_theme: 'Переключить тему',
      select_columns: 'Столбцы',
      update_application: 'Обновить приложение',
    },
    boolean: {
      true: 'Да',
      false: 'Нет',
      null: '\u2003',
    },
    page: {
      create: 'Создать %{name}',
      dashboard: 'Обзор',
      edit: '%{name} "%{recordRepresentation}"',
      error: 'Что-то пошло не так',
      list: '%{name}',
      loading: 'Загрузка',
      not_found: 'Не найдено',
      show: '%{name} "%{recordRepresentation}"',
      empty: 'В этом разделе пока нет записей.',
      invite: 'Нажмите «Создать» выше или измените фильтры, если ничего не видно.',
      access_denied: 'Доступ запрещен',
      authentication_error: 'Ошибка аутентификации',
    },
    input: {
      file: {
        upload_several: 'Перетащите файлы сюда или нажмите для выбора.',
        upload_single: 'Перетащите файл сюда или нажмите для выбора.',
      },
      image: {
        upload_several: 'Перетащите изображения сюда или нажмите для выбора.',
        upload_single: 'Перетащите изображение сюда или нажмите для выбора.',
      },
      references: {
        all_missing: 'Связанных данных не найдено',
        many_missing: 'Некоторые из связанных данных недоступны',
        single_missing: 'Связанный объект недоступен',
      },
      password: {
        toggle_visible: 'Скрыть пароль',
        toggle_hidden: 'Показать пароль',
      },
    },
    message: {
      about: 'Справка',
      access_denied: 'У вас нет необходимых разрешений для доступа к этой странице',
      are_you_sure: 'Вы уверены?',
      authentication_error:
        'Сервер аутентификации выдал сообщение об ошибке и ваши учетные данные не были проверены.',
      auth_error: 'Произошла ошибка при валидации токена аутентификации',
      bulk_delete_content:
        'Вы уверены, что хотите удалить %{name}? |||| Вы уверены, что хотите удалить %{smart_count} объекта? |||| Вы уверены, что хотите удалить %{smart_count} объектов?',
      bulk_delete_title: 'Удалить %{name} |||| Удалить %{smart_count} %{name} |||| Удалить %{smart_count} %{name}',
      bulk_update_content:
        'Вы уверены, что хотите обновить %{name}? |||| Вы уверены, что хотите обновить %{smart_count} объекта? |||| Вы уверены, что хотите обновить %{smart_count} объектов?',
      bulk_update_title: 'Обновить %{name} |||| Обновить %{smart_count} %{name} |||| Обновить %{smart_count} %{name}',
      clear_array_input: 'Вы уверены, что хотите очистить весь список?',
      delete_content: 'Запись удалится безвозвратно. Продолжить?',
      delete_title: 'Удалить %{name} "%{recordRepresentation}"',
      details: 'Описание',
      error: 'Не получилось выполнить запрос. Проверьте соединение или попробуйте ещё раз.',
      invalid_form: 'Есть поля с ошибками — прокрутите форму и исправьте подсвеченные места.',
      loading: 'Загружаем данные…',
      no: 'Нет',
      not_found: 'Ошибка URL или вы перешли по неверной ссылке',
      select_all_limit_reached:
        'Элементов слишком много, чтобы выбрать их все. Были выбраны только первые %{max} шт.',
      unsaved_changes: 'Изменения ещё не сохранены. Уйти со страницы и отменить их?',
      yes: 'Да',
      placeholder_data_warning: 'Проблема с сетью: не удалось обновить данные.',
    },
    navigation: {
      no_results: 'Ничего не нашлось по текущим условиям',
      no_more_results: 'Номер страницы %{page} вне границ. Попробуйте перейти на предыдущую страницу.',
      no_filtered_results: 'С такими фильтрами список пуст. Сбросьте фильтры или измените запрос.',
      clear_filters: 'Очистить фильтры',
      page_out_of_boundaries: 'Страница %{page} вне границ',
      page_out_from_end: 'Невозможно переместиться дальше последней страницы',
      page_out_from_begin: 'Номер страницы не может быть меньше 1',
      page_range_info: '%{offsetBegin}-%{offsetEnd} из %{total}',
      partial_page_range_info: '%{offsetBegin}-%{offsetEnd} из более %{offsetEnd}',
      current_page: 'Страница %{page}',
      page: 'На %{page} страницу',
      first: 'На первую страницу',
      last: 'На последнюю страницу',
      next: 'Следующая',
      prev: 'Предыдущая',
      previous: 'Предыдущая',
      page_rows_per_page: 'Строк на странице:',
      skip_nav: 'Перейти к содержанию',
    },
    sort: {
      sort_by: 'Сортировать в порядке %{order} по полю "%{field}"',
      ASC: 'возрастания',
      DESC: 'убывания',
    },
    auth: {
      auth_check_error: 'Пожалуйста, авторизуйтесь для продолжения работы',
      user_menu: 'Профиль',
      username: 'Имя пользователя',
      password: 'Пароль',
      email: 'Email',
      sign_in: 'Войти',
      sign_in_error: 'Ошибка аутентификации, попробуйте снова',
      logout: 'Выйти',
    },
    notification: {
      updated: 'Сохранено |||| Обновлено записей: %{smart_count} |||| Обновлено записей: %{smart_count}',
      created: 'Готово — запись создана',
      deleted: 'Удалено |||| Удалено записей: %{smart_count} |||| Удалено записей: %{smart_count}',
      bad_item: 'Элемент не валиден',
      item_doesnt_exist: 'Элемент не существует',
      http_error: 'Ошибка сервера',
      data_provider_error: 'Ошибка dataProvider, проверьте консоль',
      i18n_error: 'Не удалось загрузить перевод для указанного языка',
      canceled: 'Операция отменена',
      logged_out: 'Ваша сессия завершена, попробуйте переподключиться/войти снова',
      not_authorized: 'У вас нет доступа к этому ресурсу',
      application_update_available: 'Имеется новая версия приложения.',
      offline: 'Проблема с сетью: не удалось получить данные.',
    },
    validation: {
      required: 'Обязательно для заполнения',
      minLength: 'Минимальное кол-во символов %{min}',
      maxLength: 'Максимальное кол-во символов %{max}',
      minValue: 'Минимальное значение %{min}',
      maxValue: 'Значение может быть %{max} или меньше',
      number: 'Должно быть цифрой',
      email: 'Некорректный email',
      oneOf: 'Должно быть одним из: %{options}',
      regex: 'Должно быть в формате (regexp): %{pattern}',
      unique: 'Должно быть уникальным',
    },
    saved_queries: {
      label: 'Сохраненные запросы',
      query_name: 'Имя запроса',
      new_label: 'Сохранить текущий запрос...',
      new_dialog_title: 'Сохранить текущий запрос как',
      remove_label: 'Удалить сохраненный запрос',
      remove_label_with_name: 'Удалить запрос "%{name}"',
      remove_dialog_title: 'Удалить сохраненный запрос?',
      remove_message:
        'Вы уверены, что хотите удалить этот запрос из списка сохраненных запросов?',
      help: 'Отфильтровать список и сохранить запрос на будущее',
    },
    configurable: {
      customize: 'Настроить',
      configureMode: 'Настроить эту страницу',
      inspector: {
        title: 'Инспектор',
        content: 'Наведите на UI-элементы приложения, чтобы настроить',
        reset: 'Сбросить настройки',
        hideAll: 'Скрыть все',
        showAll: 'Показать все',
      },
      Datagrid: {
        title: 'Таблица данных',
        unlabeled: 'Безымянный столбец #%{column}',
      },
      SimpleForm: {
        title: 'Форма',
        unlabeled: 'Безымянное поле ввода #%{input}',
      },
      SimpleList: {
        title: 'Список',
        primaryText: 'Первичный текст',
        secondaryText: 'Вторичный текст',
        tertiaryText: 'Третичный текст',
      },
    },
  },
  resources: {
    products: {
      empty: 'Товаров пока нет — витрина ждёт первые карточки.',
      invite: 'Начните с названия и категории; цену и описание можно дописать позже.',
    },
    'product-categories': {
      empty: 'Категорий нет — без них товары некуда привязать.',
      invite: 'Создайте раздел каталога: название и при желании описание для себя.',
    },
    'product-variants': {
      empty: 'Вариантов пока нет (цвет, размер и т.п.).',
      invite: 'Добавьте вариант — он появится на карточке товара на сайте.',
    },
    'product-images': {
      empty: 'Фотографий пока нет.',
      invite: 'Загрузите изображения — покупателям так проще выбрать товар.',
    },
    'product-specifications': {
      empty: 'Характеристик пока нет.',
      invite: 'Добавьте строки вроде «Материал», «Габариты» — они пойдут в карточку.',
    },
    orders: {
      empty: 'Заказов пока нет. Когда клиент оформит покупку, она появится здесь.',
    },
    'callback-leads': {
      empty: 'Заявок на звонок пока не было.',
    },
    'calculator-leads': {
      empty: 'Заявок из калькулятора пока нет.',
    },
    'portfolio-projects': {
      empty: 'В портфолио пока ничего не добавлено.',
      invite: 'Добавьте объект с фото — так на сайте виден ваш опыт.',
    },
    reviews: {
      empty: 'Отзывов в базе пока нет.',
      invite: 'Создайте отзыв вручную или дождитесь публикации с витрины.',
    },
    'blog-posts': {
      empty: 'Статей в блоге пока нет.',
      invite: 'Напишите первый материал — заголовок и текст можно править в любой момент.',
    },
    'email-templates': {
      empty: 'Шаблонов писем не найдено.',
    },
    users: {
      empty: 'Пользователей с доступом пока нет (кроме вас).',
      invite: 'Добавьте учётную запись коллеге — только для тех, кому действительно нужен вход.',
    },
    groups: {
      empty: 'Групп прав пока нет.',
    },
  },
} as const

export default russianMessages
