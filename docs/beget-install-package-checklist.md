...
- Доступ к Beget проверен по SSH: `x92034sc.beget.tech`.
- Жёсткое правило работ: **только** в `~/fabrika-tentov.ru/public_html`.
- На хостинге обнаружено: **PHP 5.6.40**, **Python 3.10.12**, **MySQL 5.7**.
- Блокер запуска текущего backend: проект требует **Python 3.12+** и **MySQL 8.0+**.
- До обновления окружения выкладываем только безопасные артефакты в `public_html/_deploy/` без переключения боевого `index.php`.
...
### Что не загружаем на прод

- `.cursor/`, `docs/`, `.git/`, локальные тестовые и временные файлы, служебные архивы.
- На хостинг попадает только runtime-состав сайта и необходимые конфиги.
