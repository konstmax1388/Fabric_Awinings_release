...
# Django Admin (/admin/): по умолчанию выключен — вход только через React Admin (staff JWT). Включить: DJANGO_ADMIN_ENABLED=true.
_django_admin_enabled_env = (os.environ.get("DJANGO_ADMIN_ENABLED") or "false").strip().lower()
DJANGO_ADMIN_ENABLED = _django_admin_enabled_env in ("1", "true", "yes")

# Если DJANGO_ADMIN_ENABLED=false: опциональный редирект любого /admin/* на SPA (например http://localhost:17401/staff/).
_django_admin_redirect_url = (os.environ.get("DJANGO_ADMIN_REDIRECT_URL") or "").strip()
DJANGO_ADMIN_REDIRECT_URL = _django_admin_redirect_url or None
...
MIDDLEWARE = [
    ...
    'config.security_headers_middleware.SecurityHeadersMiddleware',
...
# Дефолты: основной репо Fabric_Awinings (17300/17301) + react_admin-стек (17302 витрина, 17401 RA, 18100 API).
_cors = os.environ.get(
    "DJANGO_CORS_ALLOWED_ORIGINS",
    "http://localhost:17300,http://127.0.0.1:17300,"
    "http://localhost:17301,http://127.0.0.1:17301,"
    "http://localhost:17302,http://127.0.0.1:17302,"
    "http://localhost:17401,http://127.0.0.1:17401,"
    "http://localhost:18100,http://127.0.0.1:18100",
)
CORS_ALLOWED_ORIGINS = [o.strip() for o in _cors.split(",") if o.strip()]

# Те же origin, что и CORS: POST форм /admin/login/ через прокси Vite (Origin = порт витрины, не API).
...
UNFOLD = {
    ...
    "SIDEBAR": {
        ...
        # reverse_lazy("admin:…") ломается, если DJANGO_ADMIN_ENABLED=false (namespace admin не подключён).
        "navigation": build_unfold_sidebar() if DJANGO_ADMIN_ENABLED else [],
    },
...
# --- Продакшен: HTTPS, HSTS, secure cookies (включается при DEBUG=False + env) ---
if not DEBUG:
    if os.environ.get("DJANGO_BEHIND_REVERSE_PROXY", "").strip().lower() in ("1", "true", "yes"):
        SECURE_PROXY_SSL_HEADER = ("HTTP_X_FORWARDED_PROTO", "https")
        USE_X_FORWARDED_HOST = True
    if os.environ.get("DJANGO_SECURE_SSL_REDIRECT", "true").strip().lower() in ("1", "true", "yes"):
    ...
    if os.environ.get("DJANGO_SECURE_HSTS_SUBDOMAINS", "true").strip().lower() in ("1", "true", "yes"):
        SECURE_HSTS_INCLUDE_SUBDOMAINS = True
    if os.environ.get("DJANGO_SECURE_HSTS_PRELOAD", "").strip().lower() in ("1", "true", "yes"):
