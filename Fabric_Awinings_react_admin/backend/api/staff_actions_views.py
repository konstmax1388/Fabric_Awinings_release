...
@extend_schema(tags=["staff"], summary="Проверка Ozon Pay (createOrder, тестовая сумма)")
class StaffOzonPayTestView(APIView):
    permission_classes = [IsAuthenticated, IsStaffUser]
    ...
    def post(self, request, *args, **kwargs):
        ...
        if not isinstance(test_result, dict):
            return Response(
                {"ok": False, "detail": "Некорректный ответ теста Ozon Pay.", "raw": None},
                status=status.HTTP_500_INTERNAL_SERVER_ERROR,
            )
        pay_url = test_result.get("redirectUrl")
        if isinstance(pay_url, str) and pay_url.strip():
            ok = True
            detail = "Ozon Pay: получена ссылка на оплату (redirectUrl)."
        else:
            msg = str(test_result.get("message") or "").strip()
            ok = False
            detail = msg or "Ozon Pay не вернул ссылку на оплату (redirectUrl)."
        raw = json.dumps(test_result, ensure_ascii=False, indent=2)
        return Response({"ok": ok, "detail": detail, "payLink": pay_url if ok else None, "raw": raw})
