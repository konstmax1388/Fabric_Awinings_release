...
class ProductDetailSerializer(ProductListSerializer):
    ...
    def get_seo(self, obj: Product) -> dict:
        ...
        from .services.seo_public import product_seo_dict

        site = self.context.get("site_settings")
        if site is None:
            site = SiteSettings.get_solo()
        return product_seo_dict(obj, site, self.context.get("request"))
...
class BlogPostDetailSerializer(BlogPostListSerializer):
    ...
    def get_seo(self, obj: BlogPost) -> dict:
        from .models import SiteSettings
        from .services.seo_public import blog_seo_dict

        site = self.context.get("site_settings")
        if site is None:
            site = SiteSettings.get_solo()
        return blog_seo_dict(obj, site, self.context.get("request"))
...
class SiteSettingsPublicSerializer(serializers.ModelSerializer):
    ...
    def get_analyticsYandex(self, obj: SiteSettings) -> dict[str, str | bool]:
        raw = (obj.yandex_metrika_counter_id or "").strip()
        digits = raw.isdigit()
        enabled = bool(obj.yandex_metrika_enabled and digits)
        return {"enabled": enabled, "counterId": raw if enabled else ""}

    def get_seoDefaults(self, obj: SiteSettings) -> dict[str, str | bool]:
        return {
            "allowIndexing": bool(obj.seo_allow_indexing),
            "region": (obj.seo_site_region or "RU").strip() or "RU",
