...
def test_staff_callback_leads_list_and_metrics(client):
    ...
    assert "orders" in m
    assert m.get("schemaVersion") == 2
    assert "byDay" in m["orders"]
    assert len(m["orders"]["byDay"]) == 14
    assert all("date" in d and "count" in d for d in m["orders"]["byDay"])
    assert "topProducts" in m
    assert isinstance(m["topProducts"], list)
    ...
    r_search = client.get(
        "/api/staff/v1/leads/callback/?search=%2B79990001122",
        HTTP_AUTHORIZATION=f"Bearer {token}",
    )
    assert r_search.status_code == 200
    assert r_search.json().get("count", 0) >= 1
