...
2. **Фаза 2 (контент)** — **частично закрыта в 2.0.4a:** API и UI для портфолио, отзывов, блога, шаблонов писем + загрузка изображений; дальше по плану — заказы/каталог и т.д.
...
3. **Фаза 5–7 (singleton sections, actions, users/groups)**
   ...
   **Статус в `Fabric_Awinings_react_admin`:** в admin-ui добавлены кастомные маршруты «Настройки сайта», «Главная страница», «Сервис» (WB/SMTP/Битрикс), расширен каталог (товар с вкладками варианты/фото/характеристики/порядок). Профили/адреса — см. [profiles-addresses-decision.md](./profiles-addresses-decision.md).
...
4. **Фаза 8 (ограничение Django Admin)**
   ...
   **Сделано в коде:** `DJANGO_ADMIN_REDIRECT_URL`, редирект `/admin/*` при `DJANGO_ADMIN_ENABLED=false` — [phase-8-django-admin-cutover.md](./phase-8-django-admin-cutover.md).
