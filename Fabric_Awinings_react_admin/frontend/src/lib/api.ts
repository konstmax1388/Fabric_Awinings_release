...
/**
 * База для fetch к Django API.
 * - Если задан VITE_API_URL — полный origin (кросс-домен / отдельный порт в dev).
 * - Иначе пустая строка: запросы на тот же origin (`/api/...`) — типичный прод за nginx и dev через proxy в vite.config.
 */
